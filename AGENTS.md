...
- ARC is in private preview. Public docs must explain that access requires approval and that a platform key alone does not enable ARC. Keep implementation-only simulation details out of public copy; never imply public availability or fabricate SDKs, production hosts, model IDs, streaming, tools, benchmarks, or launch dates.
