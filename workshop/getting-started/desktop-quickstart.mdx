...
<Steps>
  ...
  <Step title="Choose how to power the agent">
    Something that sets Desktop apart: you can power the Workshop agent with a local model running on your machine, without hosted-model inference charges. See [local model setup and privacy limits](/workshop/workshop-desktop/local-models); local inference does not make all Workshop features offline or local-only.
