...
- **No hosted inference charges** — Run inference using your own hardware
- **On-device inference** — The local model processes your prompts and context on your computer
- **Offline inference** — A downloaded model can run without an internet connection

Local inference does not make every Workshop feature offline or local-only. Network tools, hosted models, publishing, syncing, sharing, and diagnostics can still send data to external services. See [Privacy Mode settings](/workshop/core-concepts/settings#privacy-mode).
...
```bash
# Get the runtime version used by Workshop Desktop
git clone --branch b8648 --depth 1 https://github.com/ggml-org/llama.cpp
cd llama.cpp
```

Choose the build for your hardware. The NVIDIA build requires the CUDA toolkit; see the [pinned llama.cpp build instructions](https://github.com/ggml-org/llama.cpp/blob/b8648/docs/build.md#cuda) for prerequisites.
...
Then download a model that fits your available memory and start the server:
...
|---|---|
| **32 GB** | Choose a smaller model from the hardware-aware **Guided** recommendations. Qwen3-Coder-Next Q3_K exceeds this tier's memory budget; allow headroom for macOS, Workshop, and the context cache. |
