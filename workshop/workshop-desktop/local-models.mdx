...
2. **Manual mode (advanced):** Connect your own Anthropic Messages API-compatible server (like llama.cpp, LM Studio, or Ollama + adapter).
...
If you prefer to run your own inference server, Workshop can connect to any server that implements the **Anthropic Messages API** (`/v1/messages`).

The most common option is `llama-server` from [llama.cpp](https://github.com/ggml-org/llama.cpp). Other compatible servers include [LM Studio](https://lmstudio.ai/) and [Ollama](https://ollama.com/) (with an Anthropic adapter).
...
| Server | Notes |
|---|---|
| **[llama-server](https://github.com/ggml-org/llama.cpp)** (llama.cpp) | Native Anthropic Messages API support. Recommended. Supports streaming, tool use, and vision. |
| **[LM Studio](https://lmstudio.ai/)** | GUI application for running local models. Check documentation for Anthropic API compatibility settings. |
| **[Ollama](https://ollama.com/)** | Requires an Anthropic adapter (e.g., [local-openai2anthropic](https://pypi.org/project/local-openai2anthropic/)) since Ollama natively serves an OpenAI-compatible API. |
| **[vLLM](https://docs.vllm.ai/)** | High-throughput inference engine with Anthropic endpoint support. Better suited for server-class hardware. |
